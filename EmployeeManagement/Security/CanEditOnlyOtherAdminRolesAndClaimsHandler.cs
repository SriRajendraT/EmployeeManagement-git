...
{
    public class CanEditOnlyOtherAdminRolesAndClaimsHandler : AuthorizationHandler<MangeAdminRolesAndClaimsRequirement>
    {
        ...
        protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, MangeAdminRolesAndClaimsRequirement requirement)
        {
            ...
            if (context.User.IsInRole("Admin") && context.User.HasClaim(claim => claim.Type == "Edit Role" && claim.Value == "true") &&
                adminIdBeingEdited.ToLower() != loggedInAdminId.ToLower()) { context.Succeed(requirement); }
